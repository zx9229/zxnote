...
`{}`指代重复的时候。例如：`sed -r 's/^.{3}/___/g' filename`。  
`[]`表示列出范围时。例如：`sed -r 's/^[0-9]{3}/___/g' filename`。  
`()`指代正则表达式的分组时。例如：`sed -r 's/^([0-9]{3})/\1___/g' filename`。  
`+`指代一次或更多次时。例如：`sed -r 's/^[0-9]+/_/g' filename`。  
`*`指代零次或更多次时。例如：`sed -r 's/^[0-9]*/_/g' filename`。  
`*?`中的`?`指代重复任意次，但尽可能少重复。  
`??`重复0次或1次，但尽可能少重复。  
