...
```shell
# 将"文件名"以第9行为分界点切割成两份;
# 第9行归属于下一份【第一份:[beg,9); 第二份:[9,end]】;
# 命令会将"\r\n"和"\n"当成换行符，不会将"\r"当成换行符，文件中混杂了"\r\n"和"\n"也能正常处理。
# 源文件的内容会原样传送到目标文件，不会受Windows/Linux等操作系统的干扰。
csplit --keep-files 文件名 9
```

#### 合并文件  
```shell
# 按顺序读取各源文件, 将输出重定向到目标文件;
# 默认参数下, 源文件的内容会原样传送到目标文件，不会受Windows/Linux等操作系统的干扰。
cat 源文件1 源文件2 源文件3 > 目标文件
```
